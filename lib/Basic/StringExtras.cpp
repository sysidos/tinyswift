...
#include "swift/Basic/CharInfo.h"
...
void WordIterator::computeNextPosition() const {
    ...
    while (i < n && swift::isUppercase(String[i]))
        ++i;
    ...
    if (i - Position > 1) {
        ...
        while (endOfNext < n && swift::isLowercase(String[endOfNext]))
            ++endOfNext;
        ...
        else if (swift::isLowercase(String[i]))
            NextPosition = i - 1;
        else
            NextPosition = i;
    ...
    while (i < n && !swift::isUppercase(String[i]) && String[i] != '_')
        ++i;
...
void WordIterator::computePrevPosition() const {
    ...
    while (i > 0 && !swift::isUppercase(String[i - 1]) && String[i - 1] != '_')
        --i;
    ...
    if (i < effectiveEndPosition) {
        ...
        // (URL_Loader) or after the last uppercase letter if it's not (UTF_8).
        PrevPosition = i;
        if (i != 0 && swift::isLowercase(String[i]) && String[i - 1] != '_')
            --PrevPosition;
    ...
    while (i > 0 && swift::isUppercase(String[i - 1]))
        --i;
...
bool camel_case::sameWordIgnoreFirstCase(StringRef word1, StringRef word2) {
    ...
    if (swift::toLowercase(word1[0]) != swift::toLowercase(word2[0]))
        return false;
...
StringRef camel_case::toLowercaseWord(StringRef string,
    ...
    // Already lowercase.
    if (!swift::isUppercase(string[0]))
        return string;

    // Acronym doesn't get lowercased.
    if (string.size() > 1 && swift::isUppercase(string[1]))
        return string;

    // Lowercase the first letter, append the rest.
    scratch.clear();
    scratch.push_back(swift::toLowercase(string[0]));
...
StringRef camel_case::toSentencecase(StringRef string,
    ...
    // Can't be uppercased.
    if (!swift::isLowercase(string[0]))
        return string;

    // Uppercase the first letter, append the rest.
    scratch.clear();
    scratch.push_back(swift::toUppercase(string[0]));
    scratch.append(string.begin() + 1, string.end());
    return StringRef(scratch.data(), scratch.size());
}
...
StringRef camel_case::dropPrefix(StringRef string) {
    ...
    for (; firstLower < n; ++firstLower) {
        if (!swift::isUppercase(string[firstLower]))
            break;
    }
...
StringRef camel_case::appendSentenceCase(SmallVectorImpl<char> &buffer,
    ...
    // Uppercase the first letter, append the rest.
    buffer.push_back(swift::toUppercase(string[0]));
...
size_t camel_case::findWord(StringRef string, StringRef word) {
    assert(!word.empty());
    assert(swift::isUppercase(word[0]));
    ...
    while (true) {
        ...
        if (!suffix.empty() && swift::isLowercase(suffix[0]))
            continue;
...
static bool matchNameWordToTypeWord(StringRef nameWord, StringRef typeWord) {
    ...
    if (nameWord.size() < typeWord.size()) {
        ...
        if (typeWord.endswith_lower(nameWord) &&
            !swift::isLowercase(typeWord[typeWord.size() - nameWord.size()])) {
            // Check that everything preceding the match is neither a lowercase letter
            // nor a '_'.
            for (unsigned i = 0, n = nameWord.size(); i != n; ++i) {
                if (swift::isLowercase(typeWord[i]) || typeWord[i] == '_') return false;
            }
        ...
        if (typeWord.startswith_lower(nameWord)) {
            for (unsigned i = nameWord.size(), n = typeWord.size(); i != n; ++i) {
                if (!swift::isDigit(typeWord[i])) return false;
            }
...
camel_case::toLowercaseInitialisms(StringRef string,
    ...
    for (unsigned i = 0, n = string.size(); i != n; ++i) {
        ...
        if (i < n - 1 && !swift::isUppercase(string[i + 1])) {
            ...
            if (i == 0 || !swift::isLetter(string[i + 1]) ||
                isPluralSuffix(camel_case::getFirstWord(string.substr(i + 1)))) {
                scratchStr.push_back(swift::toLowercase(string[i]));
                ++i;
            }
