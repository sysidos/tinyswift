...
#ifndef SWIFT_ASSERTIMPLEMENTS_H
#define SWIFT_ASSERTIMPLEMENTS_H
...
#define ASSERT_IMPLEMENTS(DerivedClass, BaseClass, MemberName, ExpectedType)    \
  static_assert(!::std::is_same<BaseClass,                                      \
    ...
    template<class T, T *impl0, T *impl1>
    ...
    template<class T, T *impl>
    struct is_same_pointer<T, impl, impl> {
        enum {
            value = true
        };
    };
...
#endif //SWIFT_ASSERTIMPLEMENTS_H
